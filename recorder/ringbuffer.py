...
class RingBuffer():
    ...
    def push(self, floatdata):
        ...
        self.buffer[:, offset: offset + length] = floatdata[:, :]
        # second copy, can be folded
        direct = min(length, self.buffer_length - offset)
        folded = length - direct
        ...
        self.offset += length
        self.offset %= self.buffer_length
